...
{   
    ...
    {
        PTIMER_DEBUG_MSG2("No schedule in this clock cycle", p);
    } 
	...
	else if(temp_seconds == p->expiry_seconds ) 
	{
		...
		{
			temp_milliseconds = p->expiry_milliseconds - temp_milliseconds;
			
			if( ( temp_milliseconds > 0 ) && ( temp_milliseconds <  CLOCK_MILLISECOND_MAX_CONSTANT ) )
			{            
				schedule_tickless_timer_platform((p->expiry_milliseconds) % CLOCK_MILLISECOND_MAX_CONSTANT);                        
			} 
		}        
		*/	
	}
