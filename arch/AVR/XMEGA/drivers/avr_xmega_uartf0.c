#include "avr_xmega_uartf0.h"
#if defined(PLATFORM_SUPPORT_UART) && defined(UARTF0_AVAILABLE) && ( UARTF0_ENABLE == 1 )

#if ( (COMPILER == AVR_STUDIO) || ( COMPILER == WIN_AVR ) || ( COMPILER == AVR_GCC ))
	#include <avr/interrupt.h>
#endif

#if defined(UARTF0_RX_BUFFER_CONF)
	#define UARTF0_RX_BUFFER_SIZE UARTF0_RX_BUFFER_CONF
#elif defined(UARTF0_RX_BUFFER_DEFAULT)
	#define UARTF0_RX_BUFFER_SIZE UARTF0_RX_BUFFER_DEFAULT
#else
	#define UARTF0_RX_BUFFER_SIZE 64
#endif

#ifdef UARTF0_TX_INTERRUPT_ENABLE
	#if defined(UARTF0_TX_BUFFER_CONF)
		#define UARTF0_TX_BUFFER_SIZE UARTF0_TX_BUFFER_CONF
	#elif defined(UARTF0_TX_BUFFER_DEFAULT)
		#define UARTF0_TX_BUFFER_SIZE UARTF0_TX_BUFFER_DEFAULT
	#else
		#define UARTF0_TX_BUFFER_SIZE 64
	#endif
#endif

#ifdef UARTF0_TX_DISABLE
	#ifdef UARTF0_RX_DISABLE
...
#define UART_REMAP 		PORTF.REMAP
#define UART_STATUS 	USARTF0.STATUS
#define UART_CONTROL1 	USARTF0.CTRLB
#define UART_CONTROL2 	USARTF0.CTRLC
#define UART_INTERRUPT	USARTF0.CTRLA
#define UART_DATA 		USARTF0.DATA
#define UART_BAUD_LOW 	USARTF0.BAUDCTRLA
#define UART_BAUD_HIGH 	USARTF0.BAUDCTRLB
...
#define RX_BUFFER_SIZE_TEMP 					UARTF0_RX_BUFFER_SIZE
#define TX_BUFFER_SIZE_TEMP 					UARTF0_TX_BUFFER_SIZE
	...
	static ringbuf_struct_t buffer_struct_rx;
		...
		static smos_uint8_t tx_complete_flag = 0;
	#endif
#endif
...
// UART Rx Functions
	...
	static void flush(void)
	{
		...
		interrupt [USARTF0_RXC] void usartf0_rxc_isr(void)
	#endif
	{
...
// UART Tx Functions
	...
	static mos_uint8_t txComplete(void)
	{
		...
		#elif COMPILER == CODEVISION_AVR
			interrupt [USARTF0_TXC] void usartf0_txc_isr(void)
		#endif
		{
...
const struct serial_driver avr_xmega_uartf0_driver = {
	begin,
	available,
	read,
	flush,
