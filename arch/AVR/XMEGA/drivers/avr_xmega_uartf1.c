#include "avr_xmega_uartf1.h"
#if defined(PLATFORM_SUPPORT_UART) && defined(UARTF1_AVAILABLE) && ( UARTF1_ENABLE == 1 )

#if ( (COMPILER == AVR_STUDIO) || ( COMPILER == WIN_AVR ) || ( COMPILER == AVR_GCC ))
	#include <avr/interrupt.h>
#endif

#if defined(UARTF1_RX_BUFFER_CONF)
	#define UARTF1_RX_BUFFER_SIZE UARTF1_RX_BUFFER_CONF
#elif defined(UARTF1_RX_BUFFER_DEFAULT)
	#define UARTF1_RX_BUFFER_SIZE UARTF1_RX_BUFFER_DEFAULT
#else
	#define UARTF1_RX_BUFFER_SIZE 64
#endif

#ifdef UARTF1_TX_INTERRUPT_ENABLE
	#if defined(UARTF1_TX_BUFFER_CONF)
		#define UARTF1_TX_BUFFER_SIZE UARTF1_TX_BUFFER_CONF
	#elif defined(UARTF1_TX_BUFFER_DEFAULT)
		#define UARTF1_TX_BUFFER_SIZE UARTF1_TX_BUFFER_DEFAULT
	#else
		#define UARTF1_TX_BUFFER_SIZE 64
	#endif
#endif

#ifdef UARTF1_TX_DISABLE
	#ifdef UARTF1_RX_DISABLE
...
#define UART_STATUS 	USARTF1.STATUS
#define UART_CONTROL1 	USARTF1.CTRLB
#define UART_CONTROL2 	USARTF1.CTRLC
#define UART_INTERRUPT	USARTF1.CTRLA
#define UART_DATA 		USARTF1.DATA
#define UART_BAUD_LOW 	USARTF1.BAUDCTRLA
#define UART_BAUD_HIGH 	USARTF1.BAUDCTRLB
...
#define RX_BUFFER_SIZE_TEMP 					UARTF1_RX_BUFFER_SIZE
#define TX_BUFFER_SIZE_TEMP 					UARTF1_TX_BUFFER_SIZE
...
// UART Rx Functions
	...
	static void flush(void)
	{
		...
		interrupt [USARTF1_RXC] void usartf1_rxc_isr(void)
	#endif
	{
...
// UART Tx Functions
	...
	static mos_uint8_t txComplete(void)
	{
		...
		#elif COMPILER == CODEVISION_AVR
			interrupt [USARTF1_TXC] void usartf1_txc_isr(void)
		#endif
		{
...
const struct serial_driver avr_xmega_uartf1_driver = {
	begin,
	available,
	read,
	flush,
