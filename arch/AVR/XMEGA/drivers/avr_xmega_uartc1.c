#include "avr_xmega_uartc1.h"
#if defined(PLATFORM_SUPPORT_UART) && defined(UARTC1_AVAILABLE) && ( UARTC1_ENABLE == 1 )

#if ( (COMPILER == AVR_STUDIO) || ( COMPILER == WIN_AVR ) || ( COMPILER == AVR_GCC ))
	#include <avr/interrupt.h>
#endif

#if defined(UARTC1_RX_BUFFER_CONF)
	#define UARTC1_RX_BUFFER_SIZE UARTC1_RX_BUFFER_CONF
#elif defined(UARTC1_RX_BUFFER_DEFAULT)
	#define UARTC1_RX_BUFFER_SIZE UARTC1_RX_BUFFER_DEFAULT
#else
	#define UARTC1_RX_BUFFER_SIZE 64
#endif

#ifdef UARTC1_TX_INTERRUPT_ENABLE
	#if defined(UARTC1_TX_BUFFER_CONF)
		#define UARTC1_TX_BUFFER_SIZE UARTC1_TX_BUFFER_CONF
	#elif defined(UARTC1_TX_BUFFER_DEFAULT)
		#define UARTC1_TX_BUFFER_SIZE UARTC1_TX_BUFFER_DEFAULT
	#else
		#define UARTC1_TX_BUFFER_SIZE 64
	#endif
#endif

#ifdef UARTC1_TX_DISABLE
	#ifdef UARTC1_RX_DISABLE
...
#define UART_REMAP 		PORTC.REMAP
#define UART_STATUS 	USARTC1.STATUS
#define UART_CONTROL1 	USARTC1.CTRLB
#define UART_CONTROL2 	USARTC1.CTRLC
#define UART_INTERRUPT	USARTC1.CTRLA
#define UART_DATA 		USARTC1.DATA
#define UART_BAUD_LOW 	USARTC1.BAUDCTRLA
#define UART_BAUD_HIGH 	USARTC1.BAUDCTRLB
...
#define RX_BUFFER_SIZE_TEMP 					UARTC1_RX_BUFFER_SIZE
#define TX_BUFFER_SIZE_TEMP 					UARTC1_TX_BUFFER_SIZE
...
static void begin(mos_uint32_t baudrate, mos_uint8_t mode)
{
	...
	//get_uart_pins(UARTC1_PORT,&temp1,&temp2);
...
// UART Rx Functions
	...
	static void flush(void)
	{
		...
		interrupt [USARTC1_RXC] void usartc1_rxc_isr(void)
	#endif
	{
...
// UART Tx Functions
	...
	static mos_uint8_t txComplete(void)
	{
		...
		#elif COMPILER == CODEVISION_AVR
			interrupt [USARTC1_TXC] void usartc1_txc_isr(void)
		#endif
		{
...
const struct serial_driver avr_xmega_uartc1_driver = {
	begin,
	available,
	read,
	flush,
