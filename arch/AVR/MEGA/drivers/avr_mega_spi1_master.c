...
#include "avr_mega_spi1_master.h"

#if defined(PLATFORM_SUPPORT_SPI) && defined(SPI1_AVAILABLE)
#include "../../../../os/drivers/Digital.h"
#include "../../../../os/util/delay_blocking.h"

#define SPI_CONTROL_REGISTER_TEMP 	SPCR1
#define SPI_STATUS_REGISTER_TEMP 	SPSR1
#define SPI_DATA_REGISTER_TEMP 		SPDR1

static mos_uint8_t ss_pin_master_temp = 255;

static void begin(mos_uint8_t ss_pin)
{
	Digital.pinmode(SPI1_MOSI_PIN,OUTPUT);
	Digital.pinmode(SPI1_MISO_PIN,INPUT_PULLUP);
	Digital.pinmode(SPI1_SCK_PIN,OUTPUT);
...
static void end(void)
{
	...
	Digital.pinmode(SPI1_MOSI_PIN,INPUT);
	Digital.pinmode(SPI1_MISO_PIN,INPUT);
	Digital.pinmode(SPI1_SCK_PIN,INPUT);
...
const struct spi_master_driver avr_mega_spi1_master_driver = {
	begin,
	end,
	config,
	slaveSelect,
